...
import { Sparkles, Send, Gift, Plus, Share2 } from '@/components/Icons';
...
export function PremiumResultPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!qRow) {
        const { data: rowByResult } = await supabase
          .from('questions')
          .select('*')
          .eq('result_id', targetId)
          .maybeSingle();
        if (rowByResult) {
      ...
      <div className="flex-1">
        <div className="px-4 sm:px-6 pt-10 pb-8">
          ...
          {RESULT.premium.map((section, idx) => {
            const titleEmoji = SECTION_EMOJI[section.title] ?? '';
